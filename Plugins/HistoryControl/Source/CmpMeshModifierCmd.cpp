...
// @@@ handle case of tmp.mb scene. select pCube1. Construction History off then apply displaceMesh1, undo, then redo.
...
MStatus CmpMeshModifierCmd::doIt(const MDagPath &dagPath,
	const MTypeId &meshModType)
{
	...
	if (!hasHistory && !hasInternalTweaks)
	{
		MGlobal::displayWarning("Cannot add historyControl node on " + meshShapePath.fullPathName() + " as it does not have any history or tweaks");
		return MS::kFailure;
	}
	...
	if (nodeFn.attribute("inputPolymesh").isNull() || nodeFn.attribute("output").isNull())
	{
		displayError("Invalid modifier node. It doesn't have inputPolymesh and/or output attributes");
		return MS::kFailure;
	}
	...
	MPlug newStreamInMeshPlug = modNodeFn.findPlug("inputPolymesh");
	...
	if (hasInternalTweaks)
	{
		...
		MPlug inMeshModPlug = modNodeFn.findPlug("inputPolymesh");
	...
	MPlug outMeshModPlug = modNodeFn.findPlug("output");
	...
	initModifierNode(modNode, dagMods[2]);
